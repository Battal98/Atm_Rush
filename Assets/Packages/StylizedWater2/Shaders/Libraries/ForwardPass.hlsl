...
half4 ForwardPassFragment(Varyings input, FRONT_FACE_TYPE vertexFace : FRONT_FACE_SEMANTIC) : SV_Target
#else
...
#endif
{
	...
	float3 reflections = SampleReflections(reflectionVector, _ReflectionBlur, ScreenPos.xyzw, wPos, refWorldTangentNormal, viewDirNorm, reflectionPerturbation);
	...
	float4 color = float4(ApplyLighting(surfaceData, inputData, translucencyData, density, vFace), alpha);
