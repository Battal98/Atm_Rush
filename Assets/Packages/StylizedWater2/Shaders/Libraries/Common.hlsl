...
DepthData SampleDepth(float4 screenPos)
{
	...
	depth.linear01 = (depth.eye - Linear01Depth(screenPos.z, _ZBufferParams)) * (lerp(1.0, _ProjectionParams.z, unity_OrthoParams.w)); //1=orthographic
#else
	depth.raw = 1;
	depth.eye = 1;
	depth.linear01 = 1;
