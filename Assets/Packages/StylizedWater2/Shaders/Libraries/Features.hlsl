...
void BlendWaveSimulation(float3 wPos, inout float3 normals)
{
	...
	if(_WaveSimulationCoords.w == 1)
	{
		...
		float3 simulationNormals = UnpackNormal(SAMPLE_TEXTURE2D(_WaveSimulationNormals, sampler_WaveSimulationNormals, uv));
...
void SampleWaveSimulation(float3 wPos, inout float foam)
{
	...
	{
		float2 uv = BoundsToWorldUV(wPos, _WaveSimulationCoords);
		float simulationData = SAMPLE_TEXTURE2D(_WaveSimulationData, sampler_WaveSimulationData, uv).r;

		float f1 = SAMPLE_TEXTURE2D(_FoamTex, sampler_FoamTex, wPos.xz * 1).r * pow(simulationData * 2, 1);
